...
class Cnn14(nn.Module):
    def __init__(self, classes_num, linear_probe=False):
        ...
        if linear_probe:
            ...
            for param in self.fc_audioset.parameters():
    ...
    def forward(self, input, mixup_lambda=None, specaug=False):
        """
    ...
    def forward(self, input, mixup_lambda=None,
        ...
        if segment_length is None:
            ...
            segment_seq_length = segment_length / (self.time_resolution * \
                self.interpolate_ratio)
            segmentwise_outputs = []
            for i in range(math.floor(x.size(-1) / segment_seq_length)):
                start = round(i * segment_seq_length)
                end = round((i + 1) * segment_seq_length)
                (segmentwise_output, _, _) = self.att_block(x[:, :, start: end])
                segmentwise_outputs.append(segmentwise_output)
            segmentwise_outputs = torch.stack(segmentwise_outputs).transpose(0, 1)
            output_dict = {
                "clipwise_output": clipwise_output,
                "segmentwise_output": segmentwise_outputs
...
class Cnn14_DoubleGcnInter(nn.Module):
    def __init__(self, graph1, graph2, class_indices):
        
        super().__init__()
        from gcn.gcn import GcnLabelEncoderInter
...
class Cnn12_Rnn(nn.Module):
    def __init__(self, classes_num):
        
        super(Cnn12_Rnn, self).__init__()
        ...
        self.fc1 = nn.Linear(2048, 2048, bias=True)
        self.rnn = nn.GRU(2048, 1024, bidirectional=True, batch_first=True)
        self.fc_audioset = nn.Linear(2048, classes_num, bias=True)
        self.temporal_pooling = LinearSoftmax()
    ...
    def forward(self, input, mixup_lambda=None, specaug=False):
        ...
        x = F.dropout(x, p=0.2, training=self.training) # (batch_size, 2048, time_steps / 32, mel_bins / 32)
        x = torch.mean(x, dim=3)
